...
function resolveColors(variant: GlangoAvatarVariantId | null | undefined) {
  if (!variant) {
...
/** Unified Glango smiley — circle face, ring eyes, curved smile. */
export function GlangoSmileyMark({
  pixels,
  className,
  crisp = false,
  colors,
...
export function GlangoAvatarMark({
  variant = null,
  pixels = 56,
  className,
  crisp = false,
...
export function labelForAvatarVariant(variant: GlangoAvatarVariantId) {
  return getAvatarVariant(variant).labelKo;
}

/** Static SVG string for public assets (512 / 128 / wordmark). */
export const GLANGO_SMILEY_SVG_INNER = `
