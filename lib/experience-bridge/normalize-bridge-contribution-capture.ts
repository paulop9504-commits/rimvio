...
export function normalizeBridgeContributionCapture(
  ...
  return {
    ...
    fileHash: readTrimmed(input.fileHash) ?? undefined,
    geohash: readTrimmed(input.geohash) ?? undefined,
    lat: readNumber(input.lat) ?? undefined,
    lng: readNumber(input.lng) ?? undefined,
    storagePath: readTrimmed(input.storagePath) ?? undefined,
    byteSize: readNumber(input.byteSize) ?? undefined,
