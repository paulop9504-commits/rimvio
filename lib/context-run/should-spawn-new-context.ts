...
export function isExplicitContextContinue(utterance: string): boolean {
  ...
  return /(?:이\s*)?(?:맥락|작업장|컨텍스트)(?:에|에서|으로)|이어서\s*(?:해|하|짜|작업|열어|진행|만들)|여기서\s*(?:이어서|계속|작업)|지금\s*(?:이\s*)?맥락|기존\s*맥락|연결해서|이어서\s*해\s*줘|^(?:이어(?:줘|서|가|주세요)|계속(?:해|해\s*줘|진행)|다시\s*(?:열어|시작))$/iu.test(
    text,
  );
}
