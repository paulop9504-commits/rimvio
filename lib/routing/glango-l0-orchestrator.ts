...
export type GlangoL0EngineRoute =
  | "document_study"
  | "medicine"
  | "commerce_compare"
  | "receipt"
...
export type GlangoL0Result = {
  domain: GlangoL0Domain;
  secondary_domain: GlangoL0Domain | null;
  confidence: number;
  reason: string;
  action_suggested: string;
  value_metric: string;
  engine_route: GlangoL0EngineRoute;
  capture_kind: CaptureIntentKind | null;
};

export type GlangoL0LinkInput = {
  kind: "link";
  url: string;
  domain: string;
  title?: string | null;
...
export type GlangoL0CaptureInput = {
  kind: "capture";
  text: string;
  vision?: Pick<
    VisionSnapshot,
...
export type GlangoL0Input = GlangoL0LinkInput | GlangoL0CaptureInput;
...
function baseEngineRoute(kind: CaptureIntentKind | null): GlangoL0EngineRoute {
  if (!kind) {
    return "general_ocr";
  }
...
function isCommerceLink(input: GlangoL0LinkInput) {
  return (
...
function isTechCommerceLink(input: GlangoL0LinkInput) {
  return isCommerceLink(input) && isTechListingTitle(input.title, input.domain);
}

function resolveCaptureKind(input: GlangoL0CaptureInput) {
...
function routeCaptureL0(input: GlangoL0CaptureInput): GlangoL0Result {
...
function routeLinkL0(input: GlangoL0LinkInput): GlangoL0Result {
...
/**
 * Glango L0 orchestrator — unified domain JSON for link/capture ingress.
 * Pure read path; no truth mutation or network.
 */
export function routeGlangoL0(input: GlangoL0Input): GlangoL0Result {
  if (input.kind === "link") {
...
export function glangoL0ResultToJson(result: GlangoL0Result): string {
  return JSON.stringify(result);
}

export function buildGlangoL0FromOcr(input: {
  text: string;
  vision?: GlangoL0CaptureInput["vision"];
  intentKind?: CaptureIntentKind | null;
  listingPrice?: number | null;
  market?: L0ValueMarketContext | null;
}): GlangoL0Result {
  return routeGlangoL0({
    kind: "capture",
    text: input.text,
    vision: input.vision ?? null,
...
export function buildGlangoL0FromLink(input: GlangoL0LinkInput): GlangoL0Result {
  return routeGlangoL0(input);
}
