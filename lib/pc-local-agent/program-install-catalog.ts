...
const PC_VERSION_CHECK_RE =
  /버전\s*확인|몇\s*버전|이전\s*버전|구버전|최신\s*(?:으로\s*)?(?:깔|설치)|다시\s*(?:깔|설치|받)|재설치|pc\s*(?:업데이트|update)|rimvio\s*pc\s*(?:update|upgrade)/iu;
const CURSOR_RE =
  /cursor|커서\s*(설치|깔|앱|에디터)|커서에서|커서로|cursor에서/iu;
const CHROME_RE = /(?:크롬|chrome)\s*(설치|깔|다운)|브라우저\s*(설치|깔)/iu;
...
export function resolveProgramInstallOffers(utterance: string): PcProgramInstallOffer[] {
  ...
  if (text === PC_SETUP_UPDATE_QUERY || isPcSetupVersionCheckUtterance(text)) {
  ...
  if (!pcConnect && !(installVerb && (cursor || chrome || allApps))) {
    return [];
  }
...
export function isPcSetupVersionCheckUtterance(utterance: string): boolean {
  return PC_VERSION_CHECK_RE.test(utterance.trim());
}
