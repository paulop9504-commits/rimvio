...
export function compilePlatformGoal(input: {
  ...
  if (input.intent === "create") {
    ...
    const specific = wantsHotelPlatform || text.length > 30;
    if (vague && !specific) {
      return withGoalKind(
        {
          intent: input.intent,
    ...
    return withGoalKind(
      {
        intent: input.intent,
        summary: text,
        summaryKo: wantsHotelPlatform ? "호텔 예약 Platform 생성" : "새 Platform 생성",
        scope: { kind: "new_platform" },
        domain: wantsHotelPlatform ? "hotel_booking" : null,
        requestedCapabilities: wantsHotelPlatform ? [...HOTEL_BOOKING_CAPS] : [],
        flows: wantsHotelPlatform ? ["search → detail → availability → booking → payment"] : [],
        ready: true,
        clarificationKo: null,
      },
      input.intent,
