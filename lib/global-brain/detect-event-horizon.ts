import type { GlobalBrainSnapshot, EventHorizonInsight } from "@/lib/global-brain/types";
...
export function buildGlobalBrainSnapshot(input: {
  referenceDate: string;
  ...
  const sentinelTasks = remainingSchedule.filter((item) => isSentinelLikeTask(item.task));
