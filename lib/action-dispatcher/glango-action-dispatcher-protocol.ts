...
/** Glango Action Dispatcher — LLM outputs Action IDs only (No URL Rule). */
export const GLANGO_ACTION_DISPATCHER_PROTOCOL = `# Role: Glango Action Dispatcher
You are the brain of Glango Action OS. Translate user input into a standardized **Action Intent**.
...
export function buildActionDispatcherContextBlock(): string {
  return [GLANGO_ACTION_DISPATCHER_PROTOCOL, "", buildActionRegistryMarkdown()].join("\n");
}
