-- Rimvio SQL Editor · Personal Vault (migration 046)
-- Safe to re-run.

-- (paste contents of supabase/migrations/046_user_personal_vault.sql)
...
drop policy if exists "Vault owner read user_vaults" on public.user_vaults;
create policy "Vault owner read user_vaults"
  on public.user_vaults for select to authenticated
  using (auth.uid() = user_id);

drop policy if exists "Vault owner insert user_vaults" on public.user_vaults;
create policy "Vault owner insert user_vaults"
  on public.user_vaults for insert to authenticated
  with check (auth.uid() = user_id);

drop policy if exists "Vault owner update user_vaults" on public.user_vaults;
create policy "Vault owner update user_vaults"
  on public.user_vaults for update to authenticated
  using (auth.uid() = user_id)
  with check (auth.uid() = user_id);

drop policy if exists "Vault owner read user_vault_objects" on public.user_vault_objects;
create policy "Vault owner read user_vault_objects"
  on public.user_vault_objects for select to authenticated
  using (auth.uid() = user_id);

drop policy if exists "Vault owner insert user_vault_objects" on public.user_vault_objects;
...
drop policy if exists "Vault owner delete user_vault_objects" on public.user_vault_objects;
