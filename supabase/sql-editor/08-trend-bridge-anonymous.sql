-- Anonymous trend bridge — paste in Supabase SQL editor (mirrors 049_trend_bridge_anonymous.sql)
...
drop policy if exists "Users insert own trend bridge contributions"
  on public.trend_bridge_contributions;
...
drop policy if exists "Users read own trend bridge contributions"
  on public.trend_bridge_contributions;
...
drop policy if exists "Users delete own trend bridge contributions"
  on public.trend_bridge_contributions;
...
drop policy if exists "Authenticated read trend bridge rollups"
  on public.trend_bridge_rollups;
