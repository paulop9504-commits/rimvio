...
type GlangoPurpleRevealOverlayProps = {
  open: boolean;
  onClose: () => void;
};

export function GlangoPurpleRevealOverlay({
  open,
  onClose,
}: GlangoPurpleRevealOverlayProps) {
  const copy = useCopy();
  ...
  return (
    <div
      ...
      data-testid="glango-purple-reveal"
    >
      <button
        type="button"
      ...
      <div
        className={cn(
          ...
          "animate-glango-purple-reveal-in"
        )}
      >
        <div className="pointer-events-none absolute -inset-8 rounded-full bg-violet-500/25 blur-3xl animate-glango-draw-glow" />
        ...
        <div className="relative mt-4 flex size-36 items-center justify-center rounded-[2rem] bg-white shadow-2xl ring-4 ring-violet-300/50 animate-glango-draw-pop">
          <span
            ...
            aria-hidden
          />
          <GlangoAvatarMark variant="purple" pixels={112} crisp />
        </div>
