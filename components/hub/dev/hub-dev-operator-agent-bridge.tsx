...
import { runAgentTurn } from "@/lib/agent-os/agent-turn";
...
import { readActiveOperatorModelSession } from "@/lib/hub/dev/operator-model-preference";
...
export function HubDevOperatorAgentBridge(props: {
  ...
  useEffect(() => {
    ...
    void runAgentTurn({
      utterance,
      sessionId: props.draft.id ?? "hub-operator",
      hub: {
        utterance,
        draft: props.draft,
        ...
        staleGoal: null,
        modelId: readActiveOperatorModelSession()?.id ?? null,
      },
      onHubEvent: props.onLoopEvent,
    }).finally(() => {
