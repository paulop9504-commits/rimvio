...
>
> **Cursor:** Rimvio 작업 시 **`rimvio.code-workspace`** 로 이 폴더만 열 것 ([WORKSPACE.md](./WORKSPACE.md))
>
> **제품명:** Rimvio 👀
>
> **제품 정의서:** [RIMVIO_PRODUCT.md](./RIMVIO_PRODUCT.md)
>
> **아키텍처:** [RIMVIO_ARCHITECTURE.md](./RIMVIO_ARCHITECTURE.md)
>
...
`.cursor/rules/rimvio-jobs-layers.mdc`, `.cursor/rules/rimvio-enrichment.mdc` 참조.
...
**규칙 vs 구현:** `.cursor/rules/rimvio-jobs-layers.mdc`는 **현재 코드 기준** (`/` = Feed, `/stack` = Stack).
...
| `components/now-loading-shimmer.tsx` | 로딩 shimmer ("👀 림비오가...") |
...
| 파일 | 내용 |
|------|------|
| `.cursor/rules/rimvio-isolation.mdc` | **Silent Ghost와 분리** (always apply) |
| `.cursor/rules/rimvio-product.mdc` | 제품·엔지니어링 가이드 (always apply) |
| `.cursor/rules/rimvio-enrichment.mdc` | Enrichment 헌법 |
| `.cursor/rules/rimvio-jobs-layers.mdc` | L0~L5 레이어 |
...
- 제품명: **Rimvio** (2026 리브랜드; SG 코드베이스와 무관)
