...
import { acceptListingMarketHandshake } from "@/lib/globe/market/server/market-handshake-actions";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const result = await acceptListingMarketHandshake(supabase, user.id, handshakeId);
    return NextResponse.json({ ok: true, ...result });
  } catch (error) {
    const raw = extractErrorMessage(error, "accept_failed");
    const message = readMarketHandshakeUserError(raw);
    const status = raw.includes("not_registered") ? 404 : 400;
    return NextResponse.json({ error: raw, message }, { status });
  }
}
