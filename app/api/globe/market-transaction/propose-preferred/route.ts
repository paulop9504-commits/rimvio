...
import { proposeMarketTradePreferredSchedule } from "@/lib/globe/market/server/market-trade-session-server";
...
export async function POST(request: Request) {
  ...
  try {
    const session = await proposeMarketTradePreferredSchedule(supabase, user.id, {
      handshakeId,
      meetAtIso,
    });
    ...
    const message = readMarketHandshakeUserError(
      error instanceof Error ? error.message : "propose_failed",
    );
