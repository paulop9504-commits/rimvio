...
import { departMarketTradeGuest } from "@/lib/globe/market/server/market-trade-session-server";
...
export async function POST(request: Request) {
  ...
  try {
    const session = await departMarketTradeGuest(supabase, user.id, {
      handshakeId,
      lat,
      lng,
    });
    ...
    const message = readMarketHandshakeUserError(
      error instanceof Error ? error.message : "depart_failed",
    );
