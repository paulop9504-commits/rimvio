...
import { proposeMarketTradeSchedule } from "@/lib/globe/market/server/market-trade-session-server";
...
export async function POST(request: Request) {
  ...
  try {
    const session = await proposeMarketTradeSchedule(supabase, user.id, {
      handshakeId,
      meetAtIso,
      meetPlaceLabel,
    });
