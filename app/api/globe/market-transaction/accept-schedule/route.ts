...
import { acceptMarketTradeSchedule } from "@/lib/globe/market/server/market-trade-session-server";
...
export async function POST(request: Request) {
  ...
  try {
    const session = await acceptMarketTradeSchedule(supabase, user.id, {
      handshakeId,
    });
    return NextResponse.json({ ok: true, session });
  } catch (error) {
    const message = readMarketHandshakeUserError(
      error instanceof Error ? error.message : "accept_failed",
    );
