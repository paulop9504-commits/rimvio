...
import {
  removeUserProfileCover,
  uploadUserProfileCover,
} from "@/lib/peer-chat/profile-cover-server";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const coverUrl = await uploadUserProfileCover(supabase, {
      userId,
      supabaseUrl,
      bytes,
    ...
    const message =
      error instanceof Error ? error.message : "배경 사진 업로드에 실패했어요.";
...
export async function DELETE() {
  ...
  try {
    ...
    return NextResponse.json({ ok: true, coverUrl: profile?.cover_url ?? null });
  } catch (error) {
    const message =
      error instanceof Error ? error.message : "배경 사진 삭제에 실패했어요.";
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
